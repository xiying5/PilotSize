...
ui <- fluidPage(
  ...
  titlePanel("Sample Size Calculation for Pilot and Feasibility Trials"),
  p("This app calculates sample size for external pilot randomized controlled trials designed with different objectives. See paper for details about calculations. Any issues contact XX."),
               ...
               column(6,
                      wellPanel(
                        ...
                        tags$ol(
                          ...
                          tags$li("Confidence level (commonly 95%)")
                        ),
                        radioButtons("option", 
                      ...
                      wellPanel(
                        ...
                        conditionalPanel(
                          ...
                          p("To understand the extent to which participants might drop out of the definitive trial, researchers design a pilot trial to estimate the probability of drop out."),
                          tags$ol(
                            tags$li("They make an educated guess that dropout will be approximately 10%."),
                            tags$li("They aim to estimate this parameter with a precision of 10 percentage points (i.e., confidence interval width is 20 percentage points)."),
                            tags$li("They plan to use a 95% confidence interval.")
                          ),
                          p("The researchers will need to enroll 37 participants for the pilot trial (19 per group for a two-arm pilot trial) to have a 95% confidence interval with a maximum width of 20% (0% to 20%).")
                        ),
                        # Show Count example
                        conditionalPanel(
                          condition = "input.option == 'Count'",
                          p("Researchers conduct a traditional sample size calculation for the definitive trial and find they would need to enroll 180 participants over 18 months (i.e., 10 enrollments per month). Because participant recruitment is crucial for trial success, they plan to conduct a pilot trial to estimate the recruitment rate."),
                          tags$ol(
                            tags$li("The recruitment rate needed for successful enrollment in the definitive trial is 10 participants per month."),
                            tags$li("They aim to estimate this parameter with a precision of 3 (i.e., confidence interval width is 6 participants per month)."),
                            tags$li("They plan to use a 95% confidence interval.")
                          ),
                          p("To estimate this recruitment rate with the desired level of precision, researchers calculate that 44 participants (22 per group) will need to be enrolled over 4.4 months during the pilot. This calculation assumes a constant recruitment rate, and researchers might choose to extend the duration of the pilot trial if they believe this is not enough time to initiate and stabilize recruitment. ")
                        )
                      )
               ),
               ...
               column(6,
                      ...
                      wellPanel(
                        ...
                        conditionalPanel(
                          ...
                          p("Using a one-sample proportion test with a one-sided type I error of 0.05 and power of 0.95, researchers calculate a required pilot sample size of 28 participants (14 per group).")
                        ),
                        # Show Count example
                        conditionalPanel(
                          condition = "input.option1 == 'Count'",
                          p("Investigators anticipate that a definitive trial will require 240 participants and have 24-months to recruit. To assess recruitment feasibility, they plan a pilot trial. Their “goal threshold” is 10 enrollments per month, and their “minimum threshold” is 6 per month. If recruitment falls between 6 and 10, they expect that they will be able to adjust strategies to reach 10 per month and/or to extend the duration of the trial to achieve the required sample size. However, if it drops below 6, they anticipate difficulty enrolling 240 participants within the available time."),
                          p("Because the unit of observation is month rather than individuals, they calculate that an enrollment period of 6 months is needed for the pilot trial. This calculation is based on a one-sample exact test for Poisson rate with a one-sided type I error of 0.05 and power of 0.9. It assumes a constant recruitment rate, so researchers might choose to extend the duration of the pilot trial to allow recruitment to stabilize.")
                        )
                      )
               ),
               # Top Right - Input
             ...
             fluidRow(
               ...
               column(6,
                      ...
                      wellPanel(
                        ...
                        p("To ensure 95% confidence in observing at least one disagreement, the researchers calculate a sample size of 29 participants (15 per group for a two-group study) for the pilot trial.")
                        )
               ),
               column(6,
                      wellPanel(
               ...
               column(6,
                      ...
                      wellPanel(
                        ...
                        tags$ol(
                          ...
                          tags$li("They increase the pilot size by 1 participant per group and repeat the above three steps."),
                          tags$li("They discover that to minimize the combined sample size, the pilot trial should enroll 36 participants (18 participants per group) based on the UCL method.")
                        ),                      
                        )
               ),
               column(6,
                      ...
                      wellPanel(
                        ...
                        conditionalPanel(
                          ...
                          tags$ol(
                            ...
                            tags$li("They choose to use an 80% one-sided confidence interval for their calculations.")                          
                          ),
                          p("The researchers calculate that the pilot trial needs 32 participants (16 per group) to yield a one-sided 80% confidence interval of the effect size with an upper limit below 0.3 when centered around zero.")
                        ),
                        # Show binary example
                        conditionalPanel(
                          ...
                          tags$ol(
                            ...
                            tags$li("They use this 10% difference as the maximum half-width of a one-sided 80% confidence interval.")                          
                          ),
...
server <- function(input, output) {
  ...
  observeEvent(input$calc_button11, {
    ...
    if (prop < 0 || prop > 1 || ci_width1 <= 0 || ci_width1 >= 1 || ci_level1 <= 0 || ci_level1 >= 1) {
      output$n_output11 <- renderText("Error: Please ensure that 0 <= prop <= 1, 0 < ci_width < 1, and 0 < ci_level < 1.")
    } else {
      n1 <- ceiling(prec_prop(p = prop, conf.width = ci_width1*2, conf.level = ci_level1)$n)
  ...
  observeEvent(input$calc_button12, {
    ...
    # Check the correctness of input values and provide an error message
    if (rate < 0 || ci_width2 <= 0 || ci_level2 <= 0 || ci_level2 >= 1) {
      output$n_output12 <- renderText("Error: Please ensure that rate >= 0, ci_width > 0, and 0 < ci_level < 1.")
    } else {
      n2 <- prec_rate(r = rate, conf.width = ci_width2*2, conf.level = ci_level2)$time
  ...
  observeEvent(input$calc_button22, {
    ...
    if(alpha <= 0 | alpha >= 1 | beta <= 0 | beta >= 1 | lambda0 == lambda1) {
      ...
      n_exact=samplesizeOneRateExact(beta = beta, lambdaH0 = lambda0, lambda = lambda1, D = 1, alpha = alpha)$n
